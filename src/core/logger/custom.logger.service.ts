    ...
    return createLogger({
      ...
      format: format.combine(
        ...
        format.printf(({ level, timestamp, message, context, metadata, interval }) => {
          ...
          return `[BulkMandi-node] ${processId} - ${timestamp} :: ${level.toUpperCase()} [${context || 'General'}] ${message}${metaString} ${interval ? `+${interval}ms` : ''}`;
        }),
      ),
      transports: [
      ...
      write: async (log: string, _: string, callback: Function) => {
        try {
          ...
          const logEntry = new this.logModel({
            ...
            timestamp: new Date(logObj.timestamp || Date.now()),
          });
          await logEntry.save();
          callback();
        } catch (error) {
